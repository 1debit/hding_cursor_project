with user_info as(select distinct id::varchar as user_id from CHIME.FINANCE.members where id=49974451)
, t1 as(
 -- tokenization driver
    select distinct
...
t.type,
concat(case when t.user_id=pf.sender_id and pf.type_code='to_member' and t.transaction_cd != 'ADbz' then 'to member('||pf.receiver_id||'); '
       ...
       ,'; Tran_cd: ',t.transaction_cd,'; Prism Score: ', ifnull(prs.score,0)||'; dd_type: ', ifnull(t.dd_type,';') ,'-', ifnull(t.transaction_details_2,';')) as description,
...
'n/a' as decline_resp_cd,
'n/a' as vrs,
case when t.transaction_cd = 'ADbz' then  bw.policy_name||' -'||bw.decision_outcome else 'n/a' end as  rules_denied,
'n/a' as "3DS_IND_RULES",
...
left join risk.test.bwhite_OIT_txn_overview bw on bw.oit_correlation_id = t.merchant_number
...
from (with transfer as (select
    ...
    Limits:monthly_limit:remaining as Monthly_limit_remaining,
    Limits:transaction_limit as limit_txn,
    'Succeeded' as transfer_outcome,
    last_four,
    'n/a' as ERROR_REASON,
    network_code
    from
    STREAMING_PLATFORM.SEGMENT_AND_HAWKER_PRODUCTION.MOVEMONEY_HAWKER_V1_INSTANT_OUTBOUND_TRANSFER_SUCCEEDED )) 
...
0 as amt,
'n/a' as is_disputed
from (with link_fails as (select user_id , event , bin ,  original_timestamp as timestamp , id  , linked_card_id , last_four , card_type , error 
    from segment.move_money_service.debit_card_linking_failed where TRUE order by timestamp),
    ...
    select user_id , event , bin , timestamp , id , linked_card_id , last_four , card_type , error from link_fails
    UNION
    select user_id , event , bin , timestamp , id  , linked_card_id , last_four , null , null from link_successes)
where user_id in (select * from user_info)
