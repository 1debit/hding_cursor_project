-- Title: Scam Victims Demographics Analysis (Dispute-Based)
-- Intent: Extract demographic data (age, state) for confirmed P2P scam victims from disputes
-- Inputs: risk.test.hding_p2p_scam_victim_dispute_denied, chime.finance.members
-- Output: scam victim demographics with calculated age ranges and state distribution
-- Assumptions: dispute_type='p2p' indicates P2P scam victims, member data is current
-- Validation: Check for successful joins, null values, reasonable age ranges

-- Step 1: Extract scam victim demographics with age calculation
...
SELECT 
    'SCAM_VICTIMS_SUMMARY' as analysis_type,
...
FROM RISK.TEST.scam_victims_demographics;
...
-- Step 6: Join success validation
SELECT 
    'JOIN_VALIDATION' as check_type,
    total_disputes.dispute_count,
    joined_demographics.demo_count,
    joined_demographics.demo_count * 100.0 / total_disputes.dispute_count as join_success_rate
FROM (
    SELECT COUNT(DISTINCT user_id) as dispute_count
    FROM risk.test.hding_p2p_scam_victim_dispute_denied 
    WHERE dispute_type = 'p2p'
) total_disputes
CROSS JOIN (
