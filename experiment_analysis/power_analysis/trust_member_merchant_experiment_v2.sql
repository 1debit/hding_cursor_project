...
/*
>> this task is to update data tables for experiment: Trust Member Merchant Experiment;

policy: 
  https://compass.chime.com/decision-platform/experiments/378c0a1a-8156-48e0-9afa-f3a4039149b1
*/

  
/* create auth data driver*/
create or replace table rest.test.trust_member_merchant_experiment_v2 as(
with t1 as(
    -- exp dist users
    select 
        unit_id as user_id
    ...
    where experiment_key = 'trusted_member_merchant_pilot_v2'
    and _creation_timestamp::date >= '2025-01-01'
...
merge into rest.test.trust_member_merchant_experiment_v2 a
using (
    select a.user_id, a.auth_id, b.policy_name
        from rest.test.trust_member_merchant_experiment_v2 a
        inner join chime.decision_platform.card_auth_events b on (a.user_id=b.user_id and a.auth_id=b.auth_id and b.policy_name ilike '%member_trusted_merchant_v2%' and b.original_timestamp::date>='2025-02-20')
        inner join (select distinct policy_name, policy_created_at
                    from chime.decision_platform.policies
                    where 1=1
                    and policy_name ilike '%member_trusted_merchant_v2%'
...
using (
    ...
    , zeroifnull(amount_final_cred)+zeroifnull(amount_final_rev) as final_loss
        from rest.test.trust_member_merchant_experiment_v2 a
        inner join risk.prod.disputed_transactions b on (a.user_id=b.user_id and (b.authorization_code=a.auth_id or b.authorization_code=a.settled_auth_id) and b.transaction_timestamp::date>='2025-01-01')
        left join risk.prod.all_disputable_transactions c on (b.user_id=c.user_id and b.authorization_code=c.authorization_code and c.transaction_timestamp::date>='2025-01-01' and c.dispute_created_at is not null)
...
delete from rest.test.trust_member_merchant_experiment_v2_denied_auth where original_timestamp::date>=(select max(original_timestamp::date) from rest.test.trust_member_merchant_experiment_v2_denied_auth);

/* insert latest deny auth data */
insert into rest.test.trust_member_merchant_experiment_v2_denied_auth
with t1 as(
    -- exp dist users
    select 
        unit_id as user_id
